...
- Scaffold `surveys/<slug>/` up front: `protocol.md`, `prompts/`,
  `decisions/`, `sources/`, `syntheses/`, `logs/`, plus a `.gitignore`
  covering `logs/raw/`, `work/`, `candidates.tsv`, `catalog.tsv`,
...
- **Manuscript** (the paper, standalone by design — no wikilinks):
  Typst, split as `meta.typ` (title/byline/abstract, shared so renders
  can't drift) + `content.typ` (body) + `manuscript.typ` (paged
  wrapper, `surveys/style.typ`) + `manuscript-html.typ` (HTML wrapper,
  `html.elem` title block). Shape: introduction (motivation and hook),
  ...
  `references.bib`, generated by `scripts/make-bib.py` from
  `scripts/refs.tsv` (DOI content negotiation + arXiv bibtex;

# Adversarial reduction and correspondence audit
...
| Exact all-sites-observed strict affine open-cell enumeration | inherited | On the real domain restricted to exclude every classifier boundary, hyperplane-cell enumeration already proves complete duplicate-free OutputP streaming for sign cells. Ferrez et al. attain \(O(KQ\,LP(Q,D))\) for central arrangements before Rada--Černý's different incremental skeleton; neural work enumerates feasible full-dimensional activation guards and affine residuals. These cell bounds do not automatically charge construction and serialization of the survey's residual and witness fields; non-strict or boundary-inclusive fibers need separate treatment. |
| Output-sensitive full-dimensional parametric guard/residual enumeration | inherited with qualifications | Jones and Maciejowski 2006 enumerate each pLP critical-region basis once in output-linear LP-relative time; Jones--Morari and Columbano--Fukuda--Jones extend the line to sufficient-matrix pLCPs including pLP and convex pQP, with explicit general-position and perturbed per-output LP-oracle formulas. Closed critical regions can overlap on boundaries and lower-dimensional-only regions are not emitted, so these results need a boundary-ownership rule or restricted domain before satisfying the survey's disjoint all-input fiber contract. |

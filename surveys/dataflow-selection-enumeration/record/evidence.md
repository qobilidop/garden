...
One record per evidence item: a single checkable statement about the
...
Fields: **Claim** (the statement), **Works** (citekeys),
...
- **Claim:** The study design follows systematic-mapping, separate snowballing, and auditable secondary-study reporting guidance.
...
- **Claim:** Exact symbolic execution already represents behavior with path or merge guards and residual symbolic states or values.
...
- **Claim:** All-values and dependence-guided symbolic execution reduce exploration relative to selected values or dependences.
...
- **Claim:** AllSMT and disjoint projected enumeration already enumerate selected finite coordinates and projected partial models.
...
- **Claim:** Finite observer predicates induce exactly enumerable input-equivalence classes or satisfiable Boolean atoms.
...
- **Claim:** BDDs and ADDs compile exact finite-range functions with order-qualified canonicity and exponential worst cases.
...
- **Claim:** Functional-logic search uses stable shared choice identities and demand-populated fingerprints in complete or fair search schemes.
...
- **Claim:** Lazy SmallCheck refines only the partial input fragment demanded by an executable Boolean observer while preserving bounded exhaustive testing.
...
- **Claim:** Classical stream-dataflow work computes reverse demands or least legal requested-output computations for fixed inputs under explicit conditions.
...
- **Claim:** Selective functors and compositional symbolic execution establish reusable compositional structures and path summaries.
...
- **Claim:** Symbolic partial-order reduction explores representative interleavings of independent concurrent events while preserving local-property verification.
...
- **Claim:** Full-dimensional hyperplane cells have complete duplicate-free output-sensitive enumeration algorithms with the stated arithmetic or LP-relative bounds.
...
- **Claim:** Zonotope traversal supplies a rational-bit-model output-polynomial result, structured symmetry improves selected instances, and newer circuit-guided work does not replace the generic enumeration-class bound.
...
- **Claim:** Exact neural methods enumerate dense activation cells, exact input stars or polyhedra, and affine maps; activation cells and maximal equal-map regions are distinct quotients.
...
- **Claim:** Neural tree extraction constructs EC-DTs with reported exact empirical fidelity and contradiction pruning; affine decision structures prove function preservation and can remove infeasible or entailed tests.
...
- **Claim:** Hybrid-mode and piecewise-affine methods already compose guards with affine residuals and exactly merge or factor equal-behavior regions.
...
- **Claim:** Reverse search enumerates every full-dimensional pLP critical-region basis with reconstructible affine optimizer and the displayed LP-relative bound.
...
- **Claim:** Parametric programming provides a positive-semidefinite pLCP precursor, explicit sufficient-matrix output bounds, unique selected pQP representations, degeneracy-safe adjacency, rank-deficiency handling, and pLP/projection interreduction.
...
- **Claim:** OutputP, IncP, and DelayP charge input, output, prefix, and delay costs under standard hard-enumeration definitions.
...
- **Claim:** Partial-order and observer-based state reductions omit redundant interleavings or states while preserving reachable-state coverage or a declared future-event observer.
...
- **Claim:** Observation-specific redundant-state detection can prune a state when no continuation reaches a new declared code observation.
...
- **Claim:** A finite goal-driven unfolding of a finite safe Petri net can omit transitions outside every minimal configuration to a requested marking while preserving all minimal goal-reaching configurations.
...
- **Claim:** A generalized unfolding framework lets a user state which information a prefix must preserve and how cutoffs are recognized, weakens adequate orders to well-founded relations, and separates extension-selection order from cutoff correctness.
...
- **Claim:** Decision-DNNF compilation supports sound, complete, disjoint partial-model enumeration with polynomial delay after compilation.
...
- **Claim:** Bidirectional demand semantics computes least sufficient partial inputs and cost for a requested output approximation in a pure typed calculus.
- **Works:** xia2024demand
...
- **Claim:** Explicit-control methods can merge regions with different later decisions when they share a requested first action, or remove region descriptions while preserving the pointwise controller function.
...
- **Claim:** Petri-net observers characterize current or minimum initial markings consistent with an observed label word and can incorporate nondeterministic labels, timing, or unobservable events.
...
- **Claim:** Exact BDD-based bisimulation minimization can cost more than direct symbolic invariant checking and often does not improve the downstream checking problem.
...
- **Claim:** Exact neural-region work also uses star sets, explicit PWA conversion, polyhedral complexes, and parallel layerwise enumeration, with different output quotients and guarantees.
...
- **Claim:** Property-blocked and access-guided generation, bounded lazy initialization, BLISS, LISSA, and PLI refine partial inputs or heaps under bounded exhaustive-search objectives.
...
- **Claim:** Reward-space regions can group rewards that induce the same optimal MDP policy and support policy-invariance analysis.
...
- **Claim:** SymPaths records scheduler choices alongside symbolic execution and proves soundness and completeness relative to concrete executions.
...
- **Claim:** Repeated reveals/excludes and bounded equal-conflict parametric algorithms make multiplicity-sensitive event observation and query-relative reduction explicit.

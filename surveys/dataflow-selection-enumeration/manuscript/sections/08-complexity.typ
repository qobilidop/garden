...
implementing the same affine function, and piecewise-affine minimization can
merge equal-behavior guards into coarser representations @geyer2008optimal.
The selection observer retains observed equal-valued outcomes, so those
merged output counts cannot be substituted for $K$ without changing the task.
...
The hardness statements in this subsection are inherited boundaries obtained
by standard circuit and SAT encodings, not contributions of this survey.

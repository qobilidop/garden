...
Draft v1, 2026-08-08. Written by Claude Fable 5 (orchestration,
synthesis, drafting), directed and gated by Bili Dong. The frozen
method record is `baseline.md`; the auditable claim ledger is
`claims.md`; per-work evidence notes are in `sources/`. This
manuscript is the reader-facing synthesis of the same campaign.

**Abstract.** Large language models are moving into every stage of
systematic reviews, and the methodology literature is racing to keep
up. We map that literature: 1,291 works cataloged from four scholarly
indexes and one snowball round (2020–2026), 672 included and faceted
by stage, contribution, evidence, and setting, with 25 deep-read.
Four findings organize the map. The stage distribution reproduces the
...
checklists compete as disclosure in practice stays rare. And reviewer
independence for agents is undefined: no work says what makes two
agent passes independent, while the best ensemble evidence suggests
model-family diversity is what buys recall. The map itself was
conducted by an agent fleet under a human gate — the method is an
instance of its subject — and we analyze our own execution against
the norms we mapped.
...
- **RQ2 (reliability):** How is agent performance measured, and what
  do the measurements show?
...
existing reviews rather than re-screened primary studies.

## 3. Method

Full disclosure lives in `baseline.md`; this section condenses it.

**Design.** Systematic map under a pre-registered protocol
(`protocol.md`, method v0 pinned at commit d4a27ce), window
2020-01-01 to 2026-08-08, English sources.

**Search.** 11 logged queries against OpenAlex, Crossref, Semantic
Scholar, and arXiv (`logs/searches.tsv`; one query lost to a
persistent rate limit): 419 raw candidates, 412 after
arXiv–DOI dedup.

**Screening.** Two independent agent passes per candidate on
different tiers and prompt frames (pass A: Haiku-class,
criteria-framed; pass B: Sonnet-class, RQ-framed). 417 pairwise
decisions: binary agreement 0.971, Cohen's κ = 0.937, six-category
exact agreement 0.835. 33 rows (disagreements, uncertains,
single-pass) were adjudicated by the strongest tier (Fable) with
...
varied 7–40 per 56-row batch across agents and is treated as a
pool signal only.

**Deep reads.** 25 works selected by facet-guided choice for
evidence extraction: 20 read in full text (PDFs archived, transcripts
retained), 5 abstract-only. Claims resting on abstract-only evidence
are marked (A) in the ledger. Every claim below cites its evidence
note in `sources/`.

**Deviations and erratum.** Seven in-campaign deviations are logged
in `decisions/` (notably the snowball pre-filter, the single-pass
wave, and an autonomous browser-download escalation). A post-freeze
audit found ~22 residual duplicate-title groups (~26 excess rows,
~3.9% of includes) that key normalization missed
(`decisions/0008`); counts are reported as frozen.
...
| Facet classification | 672 | map.tsv |
| Deep reads (G1) | 672 | 25 notes |

## 4. RQ1 — Landscape: the pre-LLM shape, with the ends filling in
...
The strongest-documented end-to-end system is instructive for what
it does *not* contain. MedSR-Copilot (huang2026: four subagents, a
...
ablations rank two-stage extraction (−14.9pp) far above
retrieval-RAG (−3.3pp), and its absolute stage numbers (screening F1
...
setting-classified includes against medicine's 412 — the order-of-
magnitude evidence gap napoleao2021 measured has survived the LLM
transition, and SE has no guidance or norms work of its own. And
adoption runs ahead of disclosure: the field's own coordination body
reports that published reviews rarely disclose advanced AI use
(oconnor2024), even as concrete disclosed-adoption exemplars exist
(mughal2026).

## 5. RQ2 — Reliability: abundant, but mismeasured
...
**Measurement practice lags its subject.** A methodological review
of 29 LLM-screening evaluations — with Kitchenham among the authors
— found 24% reporting complete confusion matrices, 10% reporting
MCC, and 59% leaning on accuracy, which its reanalyses show is
invalid under screening's class imbalance: an accuracy-best model
lost 63.3% of relevant evidence where the cost-weighted-best lost
5.8% (madeyski2025). Even the choice between pooled and per-review
...
evidence retention (oami2025). Across corpora, cross-review variance
exceeds cross-model variance, and performance collapses exactly
where human conflict rates are high (syriani2023, huotala2025). On
appraisal instruments every individual LLM scored below every
individual human, with both degrading together as the instrument
hardens — human inter-rater κ falls from 0.84 to 0.29 (woelfle2024).
Extraction evidence is thin: one SE proof-of-concept at 87.8%
accuracy (felizardo2024 (A)), a 23-tool social-science inventory
with no pooled benchmark (legate2024), and secondhand error ranges
of 4–31% (gartlehner2025).

**Read against human baselines.** Single human reviewers run ~87–92%
screening sensitivity (range 42–100%; cited in fagerberg2025), human
extraction error reaches 50% of data elements (cited in
...
## 6. RQ3 — Norms: convergent content, fragmenting instruments
...
stage the AI performed and what the human did; describe
...
consistent: AI as a *secondary* quality-assurance reviewer,
...
conduct-and-reporting tables (fernandes2026), and a position-
statement layer (RAISE and the Cochrane-family statements,
...
## 7. RQ4 — Independence: the undefined middle
...
- A cross-family OR-ensemble (GPT-5 Thinking + Gemini 2.5 Pro, union
  ...
  vaccine-subgroup criterion at 43% sensitivity and the other
  flagged the same records (fagerberg2025). Family diversity caught
  what within-family redundancy could not.
- A 9-run consistency ensemble reached human-level appraisal
  accuracy only on items surviving unanimous agreement, deferring
  74–88% of items; the *deferral* design — human + LLM score, send
...
- High self-consistency coexists with mediocre accuracy (run-to-run
  ...
  not validity.
- The negative case: the best end-to-end system uses no redundancy
  at all (huang2026); independence mechanisms are not yet how the
  strongest pipelines buy reliability.

What no source provides is a definition: what makes two agent passes
*independent* in the sense dual human review requires — different
...
## 8. Discussion: this campaign as an instance of its subject

The map was produced by the class of system it maps, so its own
execution is fair evidence — in both directions.

**As a data point.** Same-vendor dual-pass screening across a tier
and prompt-frame split produced κ = 0.937 on a bounded binary task,
while the judgment-graded classification field varied 5.7× across
same-tier single passes — matching the literature's pattern of high
agreement on bounded tasks and degradation with judgment load
(syriani2023, woelfle2024). The adversarial verification pass
overturned ~13% of single-pass decisions, a measured error rate that
the protocol's cost asymmetry (false excludes are lost; false
includes get caught later) had wagered on.

**Against the mapped norms.** The convergent disclosure content of
RQ3 is implemented here — models, pass structure, and deviations per
stage in `baseline.md` — but this campaign inverts the one norm
every guidance source shares. The sanctioned role is AI as secondary
checker of human work; here the agent is the primary reviewer and
the human is the gate. The mapped guidance has no published norms
for that mirror image. We state it as a deliberate departure with
compensating controls: dual passes, strongest-tier adjudication,
adversarial verification with reported overturn rates, a logged
deviation record, and a human freeze gate.

**Reflexive critique.** The measurement failures RQ2 documents
appear in our own execution. We measured agreement where it was
cheapest (binary screening) and not where judgment lived —
classification ran single-pass and extraction has no second pass, the
precise gap the literature names. Key normalization bugs left ~3.9%
residual duplicates in the include set, found only by post-freeze
audit. And three deep-read pipelines autonomously escalated to a
...
(`skills/run-survey/`) — in this campaign the deviation log was
itself primary data.
...
extraction has no second pass. ~3.9% residual duplicates remain in
the frozen include set (`decisions/0008`). English only; 12 rows
undecidable on available metadata. The map is a closed baseline, not
a living review; revival criteria are recorded in `baseline.md`.

## 10. Conclusion

The LLM era has repopulated the systematic-review automation
landscape without reshaping it: screening still dominates, the
hard stages stay thin, and the medicine–SE gap persists. The
reliability literature's headline is methodological — the field
measures the wrong things more often than it measures the right ones
badly. The norms literature agrees on what to disclose and disagrees
on the form, while practice ignores both. The open problem is
independence: the strongest ensemble result says model-family
diversity buys recall, yet no one can say what "independent agent
reviewers" means. This map, produced agent-primary under a human
gate, sits exactly on that frontier — and its own deviation log
supplies the next experiment.

## References

Works deep-read by this campaign cite their evidence notes in
`sources/`; canon works link to library pages. Identifiers are
authoritative; titles for entries without recorded author strings
are derived from the works' own metadata.

- **arno2022** — Accuracy and efficiency of machine learning-assisted
  risk-of-bias assessments in real-world systematic reviews (2022).
  doi:10.7326/m22-0092 (abstract-only).
- **degen2024** — Degen, Vogel & Rzejak, "Leveraging Artificial
  Intelligence for Systematic Reviews: The FRAISR Reporting Framework
  and guidance for researchers" (2024). doi:10.31219/osf.io/ju8dk.
- **fagerberg2025** — Dual-model LLM ensemble via web chat interfaces
  reaches near-perfect sensitivity for systematic review screening
  (2025). doi:10.1101/2025.11.03.25339455 (preprint).
- **felizardo2024** — Felizardo et al., "Data extraction for
  systematic mapping study using a large language model — a
  proof-of-concept study in software engineering" (ESEM '24).
  doi:10.1145/3674805.3690743.
- **fernandes2026** — HAICO-SLR Guide: Conducting and Reporting
  Human-AI Collaboration in Systematic Literature Reviews (2026).
  doi:10.2139/ssrn.7048543 (SSRN preprint).
- **gargari2023** — Enhancing title and abstract screening for
  systematic reviews with GPT-3.5 Turbo (2023).
  doi:10.1136/bmjebm-2023-112678.
- **gartlehner2025** — Responsible integration of artificial
  intelligence in rapid reviews (RAISE context, 2025).
  doi:10.1002/cesm.70063.
- **hamel2021** — Guidance for using artificial intelligence for
  title and abstract screening while conducting knowledge syntheses.
  BMC Medical Research Methodology 21:285 (2021).
  doi:10.1186/s12874-021-01451-2.
- **holst2025** — Transparent Reporting of AI in Systematic
  Literature Reviews: Development of the PRISMA-trAIce Checklist.
  JMIR AI (2025). doi:10.2196/80247.
- **huang2026** — A PRISMA-aligned agentic framework for medical
  systematic reviews and evidence synthesis (MedSR-Copilot, 2026).
  doi:10.64898/2026.07.30.26359375 (preprint).
- **huotala2025** — Huotala, Kuutila & Mäntylä, "SESR-Eval: Dataset
...
- **legate2024** — Legate, Nimon & Noblin, "(Semi)automated
  ...
  13:664 v2, 2024). doi:10.12688/f1000research.151493.2.
- **luo2024** — Luo et al., "Potential Roles of Large Language Models
  in the Production of Systematic Reviews and Meta-Analyses" (JMIR,
  2024). doi:10.2196/56780.
- **madeyski2025** — Madeyski, Kitchenham & Shepperd, "LLM4SCREENLIT:
  Recommendations on Assessing the Performance of Large Language
  Models for Screening Literature in Systematic Reviews" (2025).
  arxiv:2511.12635v2.
- **mughal2026** — Mughal & Bilal, "LLM-Based Test Oracles:
  Source-of-Authority Taxonomy — A Systematic Literature Review"
  (2026). arxiv:2607.05031v1.
- **napoleao2021** — Napoleão, Petrillo & Hallé, "Automated Support
  for Searching and Selecting Evidence in Software Engineering: A
  Cross-domain Systematic Mapping" (SEAA 2021).
  doi:10.1109/seaa53835.2021.00015.
- **oami2025** — GPT-3.5 Turbo and GPT-4 Turbo in title and abstract
  screening for systematic reviews (2025). doi:10.2196/64682.
...
- **song2026** — Song et al., "The Phases of Living Evidence
  Synthesis Using AI" (J Med Internet Res 28:e76130, 2026).
  doi:10.2196/76130.
- **syriani2023** — Assessing the ability of ChatGPT to screen
  articles for systematic reviews (2023). arxiv:2307.06464.
- **vandinter2021** — van Dinter, Tekinerdogan & Catal, "Automation
  of systematic literature reviews: A systematic literature review"
  (Information and Software Technology 136:106589, 2021).

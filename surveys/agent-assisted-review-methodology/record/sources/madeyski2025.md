...
- **Thresholds proposed:**
  ...
  - Deployment decision rule: deploy only if Lost Evidence is at or below the pre-specified threshold (Fig 6 decision node: Lost Evidence > threshold → escalate to human review, R10); Sec 4.1's "Lower CI >0.8" wording is a slip that parses only as a Recall lower bound (Lost Evidence ≤ 20%). Genuine-prediction check: "95% lower CI of MCC > 0".
